module math_mod
  ...
  interface inverse_matrix
    ...
    module procedure inverse_matrix_r16
...
contains
  ...
  subroutine inverse_matrix_r8(A, B, ierr)
    ...
    do k = 1, n
      ...
      do j = k, n
        do i = k, n
          if (abs(B(i,j)) > tmp) then
            tmp = abs(B(i,j)); imax(k) = i; jmax(k) = j
          end if
        end do
      end do
      if (tmp + 1 == 1) then
  ...
  subroutine inverse_matrix_r16(A, B, ierr)

    real(16), intent(in ) :: A(:,:)
    real(16), intent(out) :: B(size(A,1),size(A,1))
    integer, intent(out) :: ierr
  ...
  end subroutine inverse_matrix_r16
